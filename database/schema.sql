-- StreamPanel v2 Database Schema (exported from working database)
...
-- Triggers
CREATE TRIGGER dashboard_library_preferences_updated_at
        AFTER UPDATE ON dashboard_library_preferences
        FOR EACH ROW
        BEGIN
            UPDATE dashboard_library_preferences SET updated_at = datetime('now') WHERE id = NEW.id;
        END;

CREATE TRIGGER enforce_single_subscription_per_type_insert
BEFORE INSERT ON subscriptions
FOR EACH ROW
WHEN NEW.subscription_type_id IS NOT NULL
BEGIN
  UPDATE subscriptions
  SET status = 'cancelled'
  WHERE user_id = NEW.user_id
    AND subscription_type_id = NEW.subscription_type_id
    AND status = 'active';
END;

CREATE TRIGGER iptv_panels_updated_at
AFTER UPDATE ON iptv_panels
FOR EACH ROW
BEGIN
  UPDATE iptv_panels SET updated_at = datetime('now') WHERE id = NEW.id;
END;

CREATE TRIGGER plex_packages_updated_at
AFTER UPDATE ON plex_packages
FOR EACH ROW
BEGIN
  UPDATE plex_packages SET updated_at = datetime('now') WHERE id = NEW.id;
END;
...
CREATE TRIGGER subscription_types_updated_at
AFTER UPDATE ON subscription_types
FOR EACH ROW
BEGIN
  UPDATE subscription_types SET updated_at = datetime('now') WHERE id = NEW.id;
END;
...
CREATE INDEX idx_channel_groups_active ON iptv_channel_groups(is_active);

CREATE INDEX idx_channel_groups_panel ON iptv_channel_groups(iptv_panel_id);

CREATE INDEX idx_dashboard_cached_stats_updated
            ON dashboard_cached_stats(updated_at)
        ;

CREATE INDEX idx_dashboard_library_prefs_active
        ON dashboard_library_preferences(is_active);

CREATE INDEX idx_dashboard_library_prefs_server
        ON dashboard_library_preferences(plex_server_id);
...
CREATE INDEX idx_email_logs_type ON email_logs(send_type);

CREATE INDEX idx_email_logs_user ON email_logs(user_id);

CREATE INDEX idx_email_logs_user_status ON email_logs(user_id, status);

CREATE INDEX idx_email_schedules_active ON email_schedules(is_active);
...
CREATE INDEX idx_iptv_activity_log_action ON iptv_activity_log(action);

CREATE INDEX idx_iptv_activity_log_created ON iptv_activity_log(created_at);

CREATE INDEX idx_iptv_activity_log_panel ON iptv_activity_log(iptv_panel_id);

CREATE INDEX idx_iptv_activity_log_user ON iptv_activity_log(user_id);

CREATE INDEX idx_iptv_activity_panel_created ON iptv_activity_log(iptv_panel_id, created_at);

CREATE INDEX idx_iptv_bouquets_category ON iptv_bouquets(category);

CREATE INDEX idx_iptv_bouquets_panel ON iptv_bouquets(iptv_panel_id);

CREATE UNIQUE INDEX idx_iptv_editor_playlist_id
        ON iptv_editor_playlists(playlist_id)
    ;

CREATE INDEX idx_iptv_packages_panel ON iptv_packages(iptv_panel_id);

CREATE INDEX idx_iptv_panels_active ON iptv_panels(is_active);

CREATE INDEX idx_iptv_panels_panel_type ON iptv_panels(panel_type);

CREATE INDEX idx_payment_providers_active
            ON payment_providers(is_active)
        ;

CREATE INDEX idx_playlist_channels_playlist_id
        ON iptv_editor_playlist_channels(playlist_id)
    ;

CREATE INDEX idx_plex_activity_days_since
            ON plex_user_activity(days_since_last_activity)
        ;

CREATE INDEX idx_plex_activity_server_email
            ON plex_user_activity(plex_server_id, plex_user_email)
        ;

CREATE INDEX idx_plex_activity_user_id
            ON plex_user_activity(user_id)
        ;

CREATE INDEX idx_plex_packages_active ON plex_packages(is_active);

CREATE INDEX idx_plex_servers_is_active ON plex_servers(is_active);

CREATE INDEX idx_plex_servers_server_id ON plex_servers(server_id);

CREATE INDEX idx_portal_sessions_expires
...
CREATE INDEX idx_service_requests_provisioning ON portal_service_requests(provisioning_status);

CREATE INDEX idx_service_requests_user_id ON portal_service_requests(user_id);
...
CREATE INDEX idx_subscription_plans_iptv_package
            ON subscription_plans(iptv_package_id)
        ;

CREATE INDEX idx_subscription_plans_service_type
            ON subscription_plans(service_type)
        ;

CREATE INDEX idx_subscription_types_active ON subscription_types(is_active);

CREATE INDEX idx_subscription_types_type ON subscription_types(type);
...
CREATE INDEX idx_tag_iptv_panels_panel_id
            ON tag_iptv_panels(iptv_panel_id)
        ;

CREATE INDEX idx_tag_iptv_panels_tag_id
            ON tag_iptv_panels(tag_id)
        ;

CREATE INDEX idx_tag_plex_servers_server_id
            ON tag_plex_servers(plex_server_id)
        ;

CREATE INDEX idx_tag_plex_servers_tag_id
            ON tag_plex_servers(tag_id)
        ;

CREATE INDEX idx_tags_active ON tags(is_active);

CREATE INDEX idx_user_jobs_job_id ON user_creation_jobs(job_id);

CREATE INDEX idx_user_jobs_status ON user_creation_jobs(status);

CREATE INDEX idx_user_jobs_user_id ON user_creation_jobs(user_id);

CREATE INDEX idx_user_plex_shares_server ON user_plex_shares(plex_server_id);

CREATE INDEX idx_user_plex_shares_user ON user_plex_shares(user_id);
...
CREATE INDEX idx_users_iptv_channel_group ON users(iptv_channel_group_id);

CREATE INDEX idx_users_iptv_enabled ON users(iptv_enabled);

CREATE INDEX idx_users_iptv_panel ON users(iptv_panel_id);

CREATE INDEX idx_users_iptv_panel_package ON users(iptv_panel_package_id);

CREATE INDEX idx_users_plex_enabled ON users(plex_enabled);
