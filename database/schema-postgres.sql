...
-- Migration: Add show_iptv_movies column to users if missing
DO $$ BEGIN    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'show_iptv_movies') THEN        ALTER TABLE users ADD COLUMN show_iptv_movies INTEGER DEFAULT 1;    END IF;END $$;
