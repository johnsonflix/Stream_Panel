...
const CreateUserWizard = {
    ...
    renderIPTVUserSearchResults() {
        ...
        const panelsHTML = results.results.map(result => {
            ...
            // Use YYYY-MM-DD string directly (no timestamp conversion to avoid timezone issues)
            let expirationDate = 'N/A';
            if (userData.expiration_date) {
                // YYYY-MM-DD string from NXTDashPanel (preferred - no conversion needed)
                const [year, month, day] = userData.expiration_date.split('-');
                expirationDate = `${parseInt(month)}/${parseInt(day)}/${year}`;
            } else if (userData.expire_at) {
                // 1-Stream format: ISO date string - extract YYYY-MM-DD
                const datePart = userData.expire_at.split('T')[0].split(' ')[0];
