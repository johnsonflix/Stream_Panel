...
class OneStreamPanel extends BaseIPTVPanel {
    ...
    async testConnection() {
        try {
            ...
            console.error(`❌ 1-Stream panel ${this.name} connection test failed:`, error.message);
            return false;
        }
    }
    ...
    async authenticate() {
        try {
            console.log(`🔑 Testing 1-Stream panel authentication: ${this.name}`);
            ...
            if (response.data && response.data.name !== undefined) {
                console.log(`✅ 1-Stream panel ${this.name} authenticated successfully`);
                console.log(`   Reseller: ${response.data.name}, Credits: ${response.data.credits || 0}`);
                this.authToken = 'authenticated'; // Set flag
                return true;
            }

            console.error(`❌ Unexpected response structure. Expected 'name' field but got:`, response.data);
            ...
            console.error(`❌ 1-Stream panel ${this.name} authentication failed:`, error.message);
    ...
    async createUser(username, password, packageData, bouquetIds, isTrial = false, notes = '') {
        ...
        try {
            ...
            if (usernameProvided) {
                ...
                console.log(`📝 Username will be auto-generated by panel`);
            }
            ...
            if (passwordProvided) {
                ...
                console.log(`📝 Password will be auto-generated by panel`);
            }
            ...
            console.log(`📝 Creating user on 1-Stream panel ${this.name}:`, {
            ...
            console.log(`📝 Extracted from response - line_id: ${lineId}, expire_at: ${expireAt}`);
            ...
            console.log(`🔍 Fetching actual credentials for line ${lineId} from panel...`);
            ...
            if (!createdLine) {
                ...
                if (createdUsername) {
                    console.log(`🔍 Line not found by ID, searching by username: ${createdUsername}`);
            ...
            if (!createdLine && allLines.length > 0) {
                console.log(`🔍 Trying to find most recently created line...`);
                ...
                console.log(`⚠️ Using most recent line as fallback: ${createdLine?.username}`);
            }
            ...
            console.log(`✅ Found line with actual credentials from panel`);
            console.log(`📝 Created line data:`, JSON.stringify(createdLine, null, 2));
            ...
            if (effectiveExpireAt) {
                ...
                console.log(`📅 Parsed expiration: ${effectiveExpireAt} → ${expirationTimestamp}`);
            } else {
                console.log(`📅 No expiration date (trial or unlimited user)`);
            }
    ...
    async getExtensionPackages(lineId) {
        ...
        try {
            console.log(`📦 Fetching available packages for renewal from 1-Stream panel ${this.name}...`);
            ...
            console.log(`✅ Got ${packagesData.length} packages from panel API`);
            ...
            console.error(`❌ Failed to get extension packages from panel ${this.name}:`, error.message);
            throw error;
        }
    }
    ...
    async updateUserBouquets(lineId, bouquetIds) {
        ...
        try {
            ...
            console.log(`📝 Updating bouquets for user ${lineId} on 1-Stream panel ${this.name}`);
            console.log(`📝 Setting ${bouquetIds.length} bouquets`);
            ...
            console.log(`✅ Bouquets updated successfully for user ${lineId}`);
            return response.data;

        } catch (error) {
            console.error(`❌ Failed to update bouquets for user ${lineId}:`, error.message);
            throw error;
        }
    }
    ...
    async extendUser(lineId, packageData, bouquetIds, bouquetSyncMode = 'no_change') {
        ...
        try {
            ...
            console.log(`📝 Extending user ${lineId} on 1-Stream panel ${this.name}`);
            console.log(`📝 Package ID: ${payload.package}, Bouquet Sync Mode: ${bouquetSyncMode}`);
            ...
            console.error(`❌ Failed to extend user ${lineId}:`, error.message);
    ...
    async applyBouquetSyncMode(lineId, packageId, syncMode) {
        try {
            console.log(`🔄 Applying bouquet sync mode '${syncMode}' for user ${lineId}...`);
            ...
            console.log(`📝 Current user bouquets: ${currentBouquets.length}`);
            ...
            console.log(`📝 Package bouquets: ${packageBouquets.length}`);
            ...
            switch (syncMode) {
                case 'sync_all':
                    ...
                    console.log(`📝 Sync All: Setting ${newBouquets.length} bouquets from package`);
                    break;
                ...
                case 'sync_added':
                    ...
                    console.log(`📝 Sync Added: ${newBouquets.length} bouquets (added ${newBouquets.length - currentBouquets.length} new)`);
                    break;
                ...
                case 'sync_removed':
                    ...
                    console.log(`📝 Sync Removed: ${newBouquets.length} bouquets (removed ${currentBouquets.length - newBouquets.length})`);
                    break;

                default:
                    console.log(`⚠️ Unknown sync mode '${syncMode}', skipping bouquet update`);
                    return;
            }
            ...
            if (JSON.stringify(currentBouquets.sort()) !== JSON.stringify(newBouquets.sort())) {
                ...
                console.log(`✅ Bouquets updated successfully`);
            } else {
                console.log(`ℹ️ No bouquet changes needed`);
            }

        } catch (error) {
            console.error(`❌ Failed to apply bouquet sync mode:`, error.message);
    ...
    async deleteUser(lineId) {
        ...
        try {
            console.log(`📝 Deleting user ${lineId} from 1-Stream panel ${this.name}`);
            console.log(`📝 DELETE URL: ${this.baseURL}/ext/line/${lineId}/terminate`);
            ...
            console.log(`🔍 Verifying deletion of line ${lineId}...`);
            ...
            try {
                ...
                if (stillExists) {
                    console.warn(`⚠️ WARNING: Line ${lineId} still exists on panel after deletion!`);
                    console.warn(`⚠️ Line data:`, JSON.stringify(stillExists, null, 2));
                    throw new Error(`Line ${lineId} still exists on panel after deletion attempt`);
                }

                console.log(`✅ Verified: Line ${lineId} no longer exists on panel`);
            } catch (verifyError) {
                ...
                console.warn(`⚠️ Could not verify deletion (panel may be slow to update):`, verifyError.message);
            }
    ...
    async getAllUsers(limit = 10000, offset = 0) {
        ...
        try {
            console.log(`📝 Fetching users from 1-Stream panel ${this.name} (limit: ${limit}, offset: ${offset})`);
    ...
    async getUserInfo(lineId) {
        ...
        console.log(`✅ Found user: ${user.username} (Line ID: ${user.line_id})`);
        ...
        if (user.expire_at) {
            try {
                ...
                console.log(`📅 Parsed expiration: ${user.expire_at} → ${expirationTimestamp} (${expirationDate.toISOString()})`);
            } catch (error) {
    ...
    async syncPackages() {
        ...
        try {
            console.log(`📦 Syncing packages from 1-Stream panel ${this.name}...`);
            ...
            console.log(`✅ Synced ${packages.length} packages from panel ${this.name}`);
    ...
    async syncBouquets() {
        ...
        try {
            console.log(`🔄 Syncing bouquets from 1-Stream panel ${this.name}...`);
            console.log(`📝 Note: 1-Stream embeds bouquets in packages, extracting from /ext/packages`);
            ...
            console.log(`✅ Found ${bouquetIds.length} unique bouquet IDs across all packages`);

            if (bouquetIds.length === 0) {
                console.warn('⚠️ No bouquets found in packages data');
                await this.updateHealthStatus('online');
                return [];
            }
            ...
            if (bouquetLineId) {
                console.log(`📝 Fetching bouquet names from line ${bouquetLineId}...`);
                bouquetNames = await this.fetchBouquetNamesFromLineEdit(bouquetLineId);
                console.log(`✅ Got ${Object.keys(bouquetNames).length} bouquet names from line edit page`);
            } else {
                console.log(`⚠️ No bouquet_line_id configured - bouquets will use ID-only names`);
                console.log(`💡 Configure a "Bouquet Names Line ID" in panel settings to get bouquet names`);
            }
            ...
            for (const bouquetId of bouquetIds) {
                try {
                    ...
                    if (insertedCount % 20 === 0) {
                        console.log(`📝 Inserted ${insertedCount}/${bouquetIds.length} bouquets...`);
                    }
                } catch (insertError) {
                    console.error(`❌ Failed to insert bouquet ${bouquetId}:`, insertError.message);
                }
            }
    ...
    async fetchBouquetNamesFromLineEdit(lineId) {
        try {
            ...
            console.log(`🔍 Trying /ext/bouquets endpoint...`);
            try {
                ...
                console.log(`📝 /ext/bouquets response type: ${typeof bouquetsData}`);
                ...
                if (Array.isArray(bouquetsData) && bouquetsData.length > 0) {
                    ...
                    if (Object.keys(bouquetNames).length > 0) {
                        console.log(`✅ Got ${Object.keys(bouquetNames).length} bouquet names from /ext/bouquets`);
                        const examples = Object.entries(bouquetNames).slice(0, 3);
                        examples.forEach(([id, name]) => {
                            console.log(`   📝 Bouquet ${id}: ${name}`);
                        });
                        return bouquetNames;
                    }
                }
                console.log(`📝 /ext/bouquets returned: ${JSON.stringify(bouquetsData).substring(0, 200)}`);
            } catch (bouquetError) {
                console.log(`⚠️ /ext/bouquets endpoint not available: ${bouquetError.message}`);
            }

            // Second, try getting line details which might include bouquet names
            console.log(`🔍 Trying /ext/line/${lineId} endpoint...`);
            try {
                ...
                console.log(`📝 /ext/line/${lineId} response keys: ${Object.keys(lineData || {}).join(', ')}`);
                ...
                if (lineData.bouquets && Array.isArray(lineData.bouquets)) {
                    ...
                    if (Object.keys(bouquetNames).length > 0) {
                        console.log(`✅ Got ${Object.keys(bouquetNames).length} bouquet names from line data`);
                        return bouquetNames;
                    }
                }
                console.log(`📝 Line data preview: ${JSON.stringify(lineData).substring(0, 500)}`);
            } catch (lineError) {
                console.log(`⚠️ /ext/line/${lineId} endpoint not available: ${lineError.message}`);
            }
            ...
            if (webUsername && webPassword && lineId) {
                console.log(`🔍 Trying browser session auth with web credentials...`);
                try {
                    ...
                    console.log(`⚠️ Browser session auth failed: ${webError.message}`);
                }
            } else if (!webUsername || !webPassword) {
                console.log(`⚠️ Web credentials not configured - cannot use browser session auth`);
            }

            console.log(`⚠️ Could not fetch bouquet names from API - bouquets will use ID-only names`);
            return {};

        } catch (error) {
            console.error(`❌ Failed to fetch bouquet names:`, error.message);
            return {};
        }
    }
    ...
    async fetchBouquetNamesViaWebSession(lineId, username, password) {
        ...
        try {
            ...
            console.log(`   📝 Login field detected: ${usernameField}`);
            ...
            console.log(`   📝 Fetching line edit page /lines/${lineId}/edit with Inertia header...`);
            try {
                ...
                console.log(`   📝 Inertia response type: ${typeof inertiaData}`);

                if (typeof inertiaData === 'object' && inertiaData.props) {
                    console.log(`   📝 Got Inertia JSON response, props keys: ${Object.keys(inertiaData.props || {}).join(', ')}`);
                    ...
                    for (const source of possibleBouquetSources) {
                        if (Array.isArray(source) && source.length > 0) {
                            ...
                            if (Object.keys(bouquetNames).length > 0) {
                                console.log(`   ✅ Found ${Object.keys(bouquetNames).length} bouquets in Inertia JSON response`);
                                return bouquetNames;
                            }
                        }
                    }
                    console.log(`   📝 Inertia props preview: ${JSON.stringify(props).substring(0, 500)}`);
                }
            } catch (inertiaError) {
                console.log(`   📝 Inertia request failed: ${inertiaError.message}`);
            }

            // Step 4: Fallback - fetch as regular HTML
            console.log(`   📝 Fetching line edit page /lines/${lineId}/edit as HTML...`);
            ...
            console.log(`   📝 Edit page size: ${editHtml.length} chars`);
            ...
            if (dataPageMatch) {
                try {
                    ...
                    console.log(`   📝 Found Inertia data, props keys: ${Object.keys(inertiaData.props || {}).join(', ')}`);
                    ...
                    for (const source of possibleBouquetSources) {
                        if (Array.isArray(source) && source.length > 0) {
                            ...
                            if (Object.keys(bouquetNames).length > 0) {
                                console.log(`   ✅ Found bouquets in Inertia props`);
                                break;
                            }
                        }
                    }
                    ...
                    if (Object.keys(bouquetNames).length === 0) {
                        console.log(`   📝 Available props: ${JSON.stringify(Object.keys(props))}`);
                        // Log first 500 chars of props for debugging
                        console.log(`   📝 Props preview: ${JSON.stringify(props).substring(0, 500)}`);
                    }
                } catch (e) {
                    console.log(`   ⚠️ Failed to parse Inertia data: ${e.message}`);
                }
            } else {
                console.log(`   📝 No Inertia data-page attribute found`);
            }
            ...
            if (Object.keys(bouquetNames).length > 0) {
                console.log(`✅ Got ${Object.keys(bouquetNames).length} bouquet names via web session`);
                const examples = Object.entries(bouquetNames).slice(0, 3);
                examples.forEach(([id, name]) => {
                    console.log(`   📝 Bouquet ${id}: ${name}`);
                });
            } else {
                console.log(`⚠️ No bouquet names found in edit page HTML`);
                // Log a portion of the HTML for debugging
                console.log(`   📝 HTML snippet: ${editHtml.substring(0, 500)}`);
            }

            return bouquetNames;

        } catch (error) {
            console.error(`❌ Web session auth failed:`, error.message);
            throw error;
        }
    }
    ...
    async getLiveConnections() {
        ...
        try {
            console.log(`🔍 Fetching live connections from 1-Stream panel ${this.name}...`);
            ...
            const [connectionsResponse, usersResponse] = await Promise.all([
                ...
                axios({
                    ...
                    console.warn(`⚠️ Failed to fetch users for connection enrichment: ${err.message}`);
                    return { data: [] };
                })
            ]);
            ...
            console.log(`✅ Got ${connections.length} active connections and ${users.length} users for enrichment`);
    ...
    async getDashboardStatistics() {
        try {
            console.log(`📊 Gathering dashboard statistics for 1-Stream panel ${this.name}...`);
