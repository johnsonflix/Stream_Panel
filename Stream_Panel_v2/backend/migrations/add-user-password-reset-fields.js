 ...
 *
 * Adds password reset functionality and first-time login tracking for app users
 */
...
async function migrate() {
    try {
        console.log('📂 Starting user password reset fields migration...');
        ...
        if (tokenColumn.count === 0) {
            console.log('🔄 Adding password_reset_token column to users...');
            db.db.exec(`
                ALTER TABLE users
        ...
        if (expiresColumn.count === 0) {
            console.log('🔄 Adding password_reset_expires column to users...');
            db.db.exec(`
                ALTER TABLE users
        ...
        if (firstLoginColumn.count === 0) {
            ...
            db.db.exec(`
                ALTER TABLE users
                ADD COLUMN is_first_login INTEGER DEFAULT 0
            `);
            console.log('✅ Added is_first_login column');

            // Mark all existing app users with no password as first-time login
            console.log('🔄 Marking existing users with no password as first-time login...');
            db.db.exec(`
                UPDATE users
                SET is_first_login = 1
                WHERE is_app_user = 1 AND password_hash IS NULL
            `);
